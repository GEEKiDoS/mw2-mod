...
class steam_proxy final : public component
{
public:
	...
	{
		if (this->steam_client_library_)
		{
			if (this->steam_pipe_)
			{
				if (this->global_user_)
				{
					this->steam_client_library_.invoke<void>("Steam_ReleaseUser", this->steam_pipe_, this->global_user_);
				}

				this->steam_client_library_.invoke<bool>("Steam_BReleaseSteamPipe", this->steam_pipe_);
			}
		}
	}

private:
	utils::nt::library steam_client_library_;
	...
	{
		if (!this->steam_client_library_) return nullptr;

		for (auto i = 1; i > 0; ++i)
		{
			std::string name = utils::string::va("CLIENTENGINE_INTERFACE_VERSION%03i", i);
			const auto client_engine = this->steam_client_library_
	...
	void load_client()
	{
		...
		this->steam_client_library_ = utils::nt::library::load(steam_path + "steamclient.dll");
		if (!this->steam_client_library_) return;

		this->client_engine_ = load_client_engine();
		if (!this->client_engine_) return;

		this->steam_pipe_ = this->steam_client_library_.invoke<void*>("Steam_CreateSteamPipe");
		this->global_user_ = this->steam_client_library_.invoke<void*>("Steam_ConnectToGlobalUser", this->steam_pipe_);
	...
	void clean_up_on_error()
	{
		if (this->steam_client_library_
			&& this->steam_pipe_
			&& this->global_user_
			&& this->steam_client_library_.invoke<bool>("Steam_BConnected", this->global_user_, this->steam_pipe_)
			&& this->steam_client_library_.invoke<bool>("Steam_BLoggedOn", this->global_user_, this->steam_pipe_))
		{
		...
		this->steam_client_library_ = utils::nt::library{nullptr};
	}
};
