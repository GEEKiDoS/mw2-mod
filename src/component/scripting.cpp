...
private:
	...
	{
		component_loader::get<scripting>()->start_execution();
		static_cast<void(*)()>(start_hook_.get_original())();
	}

	static void stop_execution_stub()
	{
		component_loader::get<scripting>()->stop_execution();
	                           ...
	                           game::native::VariableValue* stack)
	{
		try
		{
			...
			component_loader::get<scripting>()->dispatch(&e);
		}
		catch (std::exception& e)
		{
			propagate_error(e);
	...
	{
		component_loader::get<scripting>()->run_frame();
